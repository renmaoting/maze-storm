...
public class background6 : MonoBehaviour {
	private int level = 6;//本关是第一关,每次必须初始化
	...
	public LevelMap6 level6 = new LevelMap6();
	...
	void OnMouseDown (){
		int value = level6.GetMapValue ((int)x, (int)y);
		Debug.Log ("x:" + x +" "+ y);
		if(value == 0)
		{
			if (heroscript.walk == false && heroscript.finish == false && moneyscript.walk == false && moneyscript.finish == false) {
				level6.SetMap ((int)x, (int)y, 2);//设置地图
				heroscript.InitGame ();
				...
				if (heroscript.astar.findPath (heroscript.grid) == true && moneyscript.astar.findPath (moneyscript.grid) == true && avalueblock > 0) {
					...
					level6.SetMap ((int)x, (int)y, 0);//设置地图
				}
			}
	...
	{
		yield return new WaitForSeconds(1);
		asyn = Application.LoadLevelAsync ("level6");
		yield return new WaitForSeconds(1);
	}
