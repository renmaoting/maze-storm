...
public class background2 : MonoBehaviour {
	...
	public LevelMap2 level2 = new LevelMap2();
	...
	void OnMouseDown (){
		int value = level2.GetMapValue ((int)x, (int)y);
		Debug.Log ("x:" + x +" "+ y);
		if(value == 0)
		{
			level2.SetMap((int)x,(int)y,4);//设置地图
			heroscript.InitGame();
			...
			{
				level2.SetMap((int)x,(int)y,0);//设置地图
			}
