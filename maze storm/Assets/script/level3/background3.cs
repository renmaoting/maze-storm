...
public class background3 : MonoBehaviour {
	private int level = 3;//本关是第一关,每次必须初始化
	...
	public LevelMap3 level3 = new LevelMap3();
	...
	void OnMouseDown (){
		int value = level3.GetMapValue ((int)x, (int)y);
		Debug.Log ("x:" + x +" "+ y);
		if(value == 0)
		{
			//如果此处放下物体后路无法走通，则不可放
			if(heroscript.walk == false && heroscript.finish == false && moneyscript.walk == false && moneyscript.finish == false)
			{
				level3.SetMap((int)x,(int)y,2);//设置地图
				heroscript.InitGame();
				...
				{
					level3.SetMap((int)x,(int)y,0);//设置地图
				}
			}
		}
	}
